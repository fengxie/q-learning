...
class Agent:
    def __init__(self, lr, dim_state, dim_action, dim_hidden):
        ...
        self.input_state_ = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=[None, self.dim_state_], name="state")

        hidden_state_1 = tf.compat.v1.layers.dense(self.input_state_, self.dim_hidden_, activation=tf.nn.relu)
        hidden_state_2 = tf.compat.v1.layers.dense(hidden_state_1, self.dim_hidden_, activation=tf.nn.relu)
        self.output_ = tf.compat.v1.layers.dense(hidden_state_2, self.dim_action_, activation=tf.nn.softmax)

        self.selected_action_ = tf.argmax(input=self.output_, axis=1)

        # Backward: compute loss
        self.reward_ = tf.compat.v1.placeholder(shape=[None], dtype=tf.float32)
        self.action_ = tf.compat.v1.placeholder(shape=[None], dtype=tf.int32)

        self.indices_ = tf.range(0, tf.shape(input=self.output_)[0]) * tf.shape(input=self.output_)[1] + self.action_
        self.responsible_outputs_ = tf.gather(tf.reshape(self.output_, [-1]), self.indices_)

        self.loss_ = -tf.reduce_mean(input_tensor=tf.math.log(self.responsible_outputs_) * self.reward_)

        tvars = tf.compat.v1.trainable_variables()
        self.gradient_holders_ = []
        for index, var in enumerate(tvars):
            placeholder = tf.compat.v1.placeholder(tf.float32, name=str(index) + '_holder')
            self.gradient_holders_.append(placeholder)

        self.gradients_ = tf.gradients(ys=self.loss_, xs=tvars)
